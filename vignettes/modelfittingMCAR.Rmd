...
## Between factors precision matrix ----
  # arbitrary selection
Omega_R <- matrix(c(1.2,  0.7,  1.0,   0.2, 0.3,
                    0.7,  1.0, -1.0,   0.2, 0.4,
                    1.0, -1.0,  2.0,   0.2, 0.5,
                    0.2,  0.2,  0.2,   1.0, 0.7,
                    0.3,  0.4,  0.5,   0.7, 1.0), byrow = T, ncol = K)
...
```{r echo=F}
# this
y1 <- theta_mat[,1] %*% t(W/rowSums(W))
y2 <- theta_mat[,2] %*% t(W/rowSums(W))
y3 <- theta_mat[,3] %*% t(W/rowSums(W))
y4 <- theta_mat[,4] %*% t(W/rowSums(W))
y5 <- theta_mat[,5] %*% t(W/rowSums(W))
...
(ggplot(spat_obj$sf)+
  geom_sf(aes(fill = theta_mat[,5]))+
   scale_fill_viridis_b(n.breaks = 8))#+
# (ggplot(spat_obj$sf)+
#   geom_sf(aes(fill = y1[1,]))+
#    scale_fill_viridis_b(n.breaks = 8))+
# (ggplot(spat_obj$sf)+
#   geom_sf(aes(fill = y2[1,]))+
#    scale_fill_viridis_b(n.breaks = 8))+
# (ggplot(spat_obj$sf)+
#   geom_sf(aes(fill = y3[1,]))+
#    scale_fill_viridis_b(n.breaks = 8))+
# (ggplot(spat_obj$sf)+
#   geom_sf(aes(fill = y4[1,]))+
#    scale_fill_viridis_b(n.breaks = 8))+
# (ggplot(spat_obj$sf)+
#   geom_sf(aes(fill = y5[1,]))+
#    scale_fill_viridis_b(n.breaks = 8))
```

## Model fitting

Fitting the model with Stan is simple. First we compile the full Stan model stored externally, before running the `sampling` function.
...
```{r}
# get summary of posterior draws
summ <- summary(fit_e)$summary %>% 
  as.data.frame() %>% 
  mutate(n_eff_per_min = n_eff/rtmins_e) %>% 
  rownames_to_column("parameter")

# Compare rho and correlation matrix
summ %>% filter(str_detect(parameter, "rho|sd_R|Cor_R\\["))
rho
Cor_R

# Compare marginal variances
sqrt(diag(Sigma_R))
summ %>% filter(str_detect(parameter, "sd_R"))
```
