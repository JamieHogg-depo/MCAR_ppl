...
\end{align}
$$
Above we're shown how we partition $\jdist{log}{ \left| \mathbf{\Omega}^A \right| }$ into three linear components. Given that the weight matrix, and thus, $\mathbf{D}$ are fixed, the first term is a constant that can be passed as data to Stan. Following work by Donegan [@Donegan2021], the second term, $\jdist{log}{ \left| \mathbf{I} - \rho \mathbf{J} \right| }$, can be efficiently calculated using,

$$
\sum_{i=1}^M \jdist{log}{ 1 - \rho \lambda_i },
$$
where $\lambda_i$ is the $i$th eigenvalue of $\mathbf{J}$. Once again $\mathbf{J}$ is fixed and thus the eigenvalues can be passed to Stan as data. To support this formula, consider the following example.  
...
determinant(I - rho * J, logarithm = TRUE)$modulus
```
...
\underbrace{\mathbf{A}^R}_{K \times M} &= \mathbf{\Omega}^R \mathbf{Y}
\\
\underbrace{\mathbf{A}^{*S}}_{M \times K} &= \lb{ \mathbf{I}-\rho\mathbf{W} } \mathbf{Y}^T
\\
(\mathbf{y})' \mathbf{\Omega}^A (\mathbf{y}) &= \sum \jdist{diag}{\mathbf{D}} \times \jdist{diag}{ \mathbf{A}^{*S} \mathbf{A}^R },
\end{align}
$$
where $\times$ is componentwise multiplication of the two vectors. Consider the following example,
...
s=Sys.time()
A_S <- (I - rho * J) %*% Y2; A_R <- Omega_R %*% t(Y2)
...
UP TO HERE! NOTHING WORKS BELOW --- YET!
...
* @param J_eigenvalues eigenvalues of J
...
real MCARnet1_lpdf(
	...
	matrix J, 					// J matrix (M x M)
	vector J_eigenvalues,		// eigenvalues for J
	...
	int K) { 					// Number of factors
		...
		for(i in 1:M){
			...
			ldet_J[i] = log1m( rho * C_eigenvalues[i] ); 
		}
		return -0.5 * ( 
		M*K*log( 2 * pi() ) 
		+  ( 2 * M * sum( log_d_Sigma_R_chol ) - K * sum( ldet_J ) ) + sq_term 
		);
}
}
...
* @param rho Spatial dependence parameter
* @param J matrix that represents spatial neighborhood 
* @param J_eigenvalues eigenvalues of C
* @param M number of areas
* @param K number of factors
**
@return Log probability density
...
real MCARt1_lpdf(
	...
	int [] J_v , 
	int [] J_u , 
	int [] offD_id_J_w ,		// indexes for off diagonal terms
	int [] D_id_J_w , 		// indexes for diagonal terms - length M
	vector J_eigenvalues,		// eigenvalues for C
	int M, 						// Number of areas
	int K) { 					// Number of factors
		vector[M] ldet_J;
		vector[K] log_d_Sigma_R_chol = log(diagonal(cholesky_decompose(Sigma_R)));
		...
		// using crs_matrix_times_vector
			vector [ num_elements(J_w) ] ImrhoC;
			matrix[M, K] A_S;
			// Multiple off-diagonal elements by rho
			ImrhoC [ offD_id_J_w ] = - rho * J_w[ offD_id_J_w ];
			// Calculate diagonal elements of ImrhoJ
			ImrhoC [ D_id_J_w ] = 1 - rho * J_w[ D_id_J_w ];
			for(k in 1:K){
				A_S[,k] = csr_matrix_times_vector( M, M, ImrhoC, J_v, J_u, x_mat[,k] );
			}
		real sq_term = sum( A_R .* A_S' );
		for(i in 1:M){
			// equivelant to log(1-rho * lambda[i])
			ldet_J[i] = log1m( rho * J_eigenvalues[i] ); 
		}
		return -0.5 * ( 
		M*K*log( 2 * pi() ) 
		+  ( 2 * M * sum( log_d_Sigma_R_chol ) - K * sum( ldet_J ) ) + sq_term 
